...
func (t *TestSuite) AssertActiveRootBlocks(expectedBlocks []*blocks.Block, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			activeRootBlocks := node.Protocol.MainEngine.Get().EvictionState.ActiveRootBlocks()
...
func (t *TestSuite) AssertEvictedSlot(expectedIndex iotago.SlotIndex, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if expectedIndex != lo.Return1(node.Protocol.MainEngine.Get().EvictionState.LastEvictedSlot()) {
				return ierrors.Errorf("AssertEvictedSlot: %s: expected %d, got %d", node.Name, expectedIndex, lo.Return1(node.Protocol.MainEngine.Get().EvictionState.LastEvictedSlot()))
			}
