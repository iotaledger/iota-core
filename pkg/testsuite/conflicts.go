	...
	for _, node := range nodes {
		for _, conflictAlias := range expectedConflictAliases {
			t.Eventually(func() error {
				acceptanceState := node.Protocol.MainEngineInstance().Ledger.ConflictDAG().AcceptanceState(ds.NewSet(t.TransactionFramework.SignedTransactionID(conflictAlias)))
