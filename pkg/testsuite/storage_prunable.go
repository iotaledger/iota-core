...
func (t *TestSuite) AssertPrunedUntil(expectedStorage *types.Tuple[int, bool],
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if err := t.assertPrunedUntil(node.Protocol.MainEngine.Get().Storage, expectedStorage, expectedDecidedUpgrades, expectedPoolStats, expectedCommittee, expectedRewards); err != nil {
