	...
	for _, node := range nodes {
		t.Eventually(func() error {
			...
			for version, expectedEpoch := range epochVersions {
				epochForVersion, exists := node.Protocol.MainEngine().Storage.Settings().APIProvider().EpochForVersion(version)
				if !exists {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			...
			for version, expectedProtocolParameters := range versionsAndProtocolParameters {
				protocolParameters := node.Protocol.MainEngine().Storage.Settings().APIProvider().ProtocolParameters(version)
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			...
			for version, expectedProtocolParametersHash := range versionsAndProtocolParametersHashes {
				protocolParametersHash := node.Protocol.MainEngine().Storage.Settings().APIProvider().ProtocolParametersHash(version)
