...
func (t *TransactionFramework) RegisterTransaction(alias string, transaction *iotago.Transaction) {
	...
	for outputID, output := range lo.PanicOnErr(transaction.OutputsSet()) {
		...
		t.states[fmt.Sprintf("%s:%d", alias, outputID.Index())] = utxoledger.CreateOutput(t.api, actualOutputID, iotago.EmptyBlockID(), 0, t.api.SlotTimeProvider().SlotIndexFromTime(time.Now()), clonedOutput)
	}
}
