...
func (t *TestSuite) AssertSnapshotImported(imported bool, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if imported != node.Protocol.MainEngine.Get().Storage.Settings().IsSnapshotImported() {
				return ierrors.Errorf("AssertSnapshotImported: %s: expected %v, got %v", node.Name, imported, node.Protocol.MainEngine.Get().Storage.Settings().IsSnapshotImported())
			}
...
func (t *TestSuite) AssertLatestCommitment(commitment *iotago.Commitment, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if !commitment.Equals(node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().Commitment()) {
				return ierrors.Errorf("AssertLatestCommitment: %s: expected %s, got %s", node.Name, commitment, node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment())
			}
...
func (t *TestSuite) AssertCommitmentSlotIndexExists(slot iotago.SlotIndex, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().ID().Slot() < slot {
			...
			cm, err := node.Protocol.MainEngine.Get().Storage.Commitments().Load(slot)
			if err != nil {
...
func (t *TestSuite) AssertLatestCommitmentSlotIndex(slot iotago.SlotIndex, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if slot != node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().Slot() {
				return ierrors.Errorf("AssertLatestCommitmentSlotIndex: %s: expected %v, got %v", node.Name, slot, node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().Slot())
			}
...
func (t *TestSuite) AssertLatestCommitmentCumulativeWeight(cw uint64, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if cw != node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().CumulativeWeight() {
				return ierrors.Errorf("AssertLatestCommitmentCumulativeWeight: %s: expected %v, got %v", node.Name, cw, node.Protocol.MainEngine.Get().Storage.Settings().LatestCommitment().CumulativeWeight())
			}
...
func (t *TestSuite) AssertLatestFinalizedSlot(slot iotago.SlotIndex, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if slot != node.Protocol.MainEngine.Get().Storage.Settings().LatestFinalizedSlot() {
				return ierrors.Errorf("AssertLatestFinalizedSlot: %s: expected %d, got %d from settings", node.Name, slot, node.Protocol.MainEngine.Get().Storage.Settings().LatestFinalizedSlot())
			}
