...
func (t *TestSuite) AssertProtocolParameters(parameters iotago.ProtocolParameters, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			if !parameters.Equals(node.Protocol.MainEngine().CurrentAPI().ProtocolParameters()) {
				return ierrors.Errorf("AssertProtocolParameters: %s: expected %s, got %s", node.Name, parameters, node.Protocol.MainEngine().CurrentAPI().ProtocolParameters())
			}
...
func (t *TestSuite) AssertCommitmentSlotIndexExists(slot iotago.SlotIndex, nodes ...*mock.Node) {
	...
	for _, node := range nodes {
		t.Eventually(func() error {
			...
			if node.Protocol.Chains.MainChain().LatestCommitment().ID().Index() < slot {
