		...
		return options.Apply(&Clock{
			...
			e.HookConstructed(func() {
				e.Storage.Settings().HookInitialized(func() {
					c.acceptedTime.Set(e.API().SlotTimeProvider().SlotEndTime(e.Storage.Settings().LatestCommitment().Index()))
					c.confirmedTime.Set(e.API().SlotTimeProvider().SlotEndTime(e.Storage.Settings().LatestFinalizedSlot()))
				...
				c.HookStopped(lo.Batch(
					...
					e.Events.SlotGadget.SlotFinalized.Hook(func(index iotago.SlotIndex) {
						c.acceptedTime.Advance(e.API().SlotTimeProvider().SlotEndTime(index))
						c.confirmedTime.Advance(e.API().SlotTimeProvider().SlotEndTime(index))
					}, asyncOpt).Unhook,
				))
			})
