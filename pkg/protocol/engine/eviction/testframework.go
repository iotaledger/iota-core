...
func (t *TestFramework) RequireLastEvictedSlot(expectedSlot iotago.SlotIndex, expectedHasEvicted bool) {
	slot, hasEvicted := t.Instance.LastEvictedSlot()
	require.Equal(t.Testing, expectedSlot, slot)
	require.Equal(t.Testing, expectedHasEvicted, hasEvicted)
}
