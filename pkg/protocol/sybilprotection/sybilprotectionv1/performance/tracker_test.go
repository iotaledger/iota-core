...
func TestManager_Candidates(t *testing.T) {
	...
	{
		block1 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)

		block1.IssuingTime = ts.api.ProtocolParameters().TimeProvider().SlotStartTime(1)
		block1.IssuerID = issuer1

		block2 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)

		block2.IssuingTime = ts.api.ProtocolParameters().TimeProvider().SlotStartTime(2)
		block2.IssuerID = issuer2

		block3 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)
	...
	{
		block4 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)
		block4.IssuingTime = ts.api.ProtocolParameters().TimeProvider().SlotStartTime(4)
		block4.IssuerID = issuer1

		block5 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)

		block5.IssuingTime = ts.api.ProtocolParameters().TimeProvider().SlotStartTime(5)
		block5.IssuerID = issuer2

		block6 := tpkg.RandProtocolBlock(tpkg.RandBasicBlock(ts.api, iotago.PayloadSignedTransaction), ts.api, 0)
