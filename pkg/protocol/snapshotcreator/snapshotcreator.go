...
func createGenesisAccounts(accounts []AccountDetails, engineInstance *engine.Engine, protocolParams *iotago.ProtocolParameters) (err error) {
	...
	for idx, account := range accounts {
		output := createAccount(account.Address, account.Amount, account.Mana, account.IssuerKey, account.StakedAmount, account.StakingEpochEnd, account.FixedCost)
...
func createAccount(address iotago.Address, tokenAmount iotago.BaseToken, mana iotago.Mana, pubkey ed25519.PublicKey, stakedAmount iotago.BaseToken, stakeEndEpoch iotago.EpochIndex, stakeFixedCost iotago.Mana) (output iotago.Output) {
