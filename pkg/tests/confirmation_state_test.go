...
func TestConfirmationFlags(t *testing.T) {
	...
	ts.AssertNodeState(ts.Nodes(),
		...
		testsuite.WithSybilProtectionOnlineCommittee(lo.Return1(nodeA.Protocol.Engines.Main.Get().SybilProtection.SeatManager().Committee(1).GetSeat(nodeA.Validator.AccountID))),
	...
	// Slots 1-3: only node A is online and issues blocks, make slot 1 committed.
	{
		...
		ts.AssertNodeState(ts.Nodes(),
			...
			testsuite.WithSybilProtectionOnlineCommittee(
				lo.Return1(nodeA.Protocol.Engines.Main.Get().SybilProtection.SeatManager().Committee(1).GetSeat(nodeA.Validator.AccountID)),
				lo.Return1(nodeA.Protocol.Engines.Main.Get().SybilProtection.SeatManager().Committee(1).GetSeat(nodeB.Validator.AccountID)),
				lo.Return1(nodeA.Protocol.Engines.Main.Get().SybilProtection.SeatManager().Committee(1).GetSeat(nodeC.Validator.AccountID)),
			),
			testsuite.WithEvictedSlot(2),
		)
