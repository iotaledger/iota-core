...
// TODO: end of new part
		...
		// Slot 9-12
		{
			...
			ts.AssertBlocksInCacheAccepted(ts.Blocks("9.1", "9.2"), true, ts.Nodes()...)
			ts.AssertBlocksInCacheConfirmed(ts.Blocks("9.1", "9.2"), false, ts.Nodes()...) // too old. confirmation ratification threshold = 2
		}
	...
	// Load node3 from created snapshot and verify state.
	{
		...
		ts.AssertNodeState(ts.Nodes("node3"),
			...
			testsuite.WithEvictedSlot(10),
			testsuite.WithActiveRootBlocks(ts.Blocks("8.2", "9.2", "10.2")),
			testsuite.WithStorageRootBlocks(ts.Blocks("8.2", "9.2", "10.2")),
