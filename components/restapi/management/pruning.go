...
func pruneDatabase(c echo.Context) (*apimodels.PruneDatabaseResponse, error) {
	if deps.Protocol.MainEngine.Get().Storage.IsPruning() {
	...
	if request.Index != 0 {
		err = deps.Protocol.MainEngine.Get().Storage.PruneByEpochIndex(request.Index)
		if err != nil {
	...
	if request.Depth != 0 {
		_, _, err := deps.Protocol.MainEngine.Get().Storage.PruneByDepth(request.Depth)
		if err != nil {
			return nil, ierrors.Wrapf(echo.ErrInternalServerError, "pruning database failed: %s", err)
		}
	}
	...
	if request.TargetDatabaseSize != "" {
		...
		err = deps.Protocol.MainEngine.Get().Storage.PruneBySize(pruningTargetDatabaseSizeBytes)
		if err != nil {
			return nil, ierrors.Wrapf(echo.ErrInternalServerError, "pruning database failed: %s", err)
		}
	}

	targetIndex, hasPruned := deps.Protocol.MainEngine.Get().Storage.LastPrunedEpoch()
	if hasPruned {
		targetIndex++
	}
