...
func NewOutput(output iotago.Output) (result *Output) {
	outputJSON, err := deps.Protocol.MainEngine().CurrentAPI().JSONEncode(output)
	if err != nil {
		return nil
	}
...
func NewTransaction(iotaTx *iotago.Transaction) *Transaction {
	txID, err := iotaTx.ID(deps.Protocol.MainEngine().CurrentAPI())
	if err != nil {
		return nil
	}
	...
	if iotaTx.Essence.Payload != nil {
		dataPayload, _ = deps.Protocol.MainEngine().CurrentAPI().Encode(iotaTx.Essence.Payload)
	}
...
func NewUnlockBlock(unlockBlock iotago.Unlock) *UnlockBlock {
	...
	case *iotago.SignatureUnlock:
		...
		case *iotago.Ed25519Signature:
			sigJSON, err := deps.Protocol.MainEngine().CurrentAPI().JSONEncode(signature)
			if err != nil {
				return nil
			}
