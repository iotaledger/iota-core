...
var SchedulerMetrics = collector.NewCollection(schedulerNamespace,
	collector.WithMetric(collector.NewMetric(queueSizePerNodeWork,
		...
		collector.WithInitFunc(func() {
			deps.Protocol.Events.Engine.Scheduler.BlockEnqueued.Hook(func(block *blocks.Block) {
				deps.Collector.Update(schedulerNamespace, queueSizePerNodeWork, float64(deps.Protocol.MainEngine.Get().Scheduler.IssuerQueueWork(block.ProtocolBlock().IssuerID)), block.ProtocolBlock().IssuerID.String())
		...
		collector.WithInitFunc(func() {
			deps.Protocol.Events.Engine.Scheduler.BlockEnqueued.Hook(func(block *blocks.Block) {
				if _, isBasic := block.BasicBlock(); isBasic {
					deps.Collector.Update(schedulerNamespace, queueSizePerNodeCount, float64(deps.Protocol.MainEngine.Get().Scheduler.IssuerQueueBlockCount(block.ProtocolBlock().IssuerID)), block.ProtocolBlock().IssuerID.String())
				}
			}, event.WithWorkerPool(Component.WorkerPool))
		...
		collector.WithInitFunc(func() {
			deps.Protocol.Events.Engine.Scheduler.BlockEnqueued.Hook(func(block *blocks.Block) {
				if _, isValidation := block.ValidationBlock(); isValidation {
					deps.Collector.Update(schedulerNamespace, validatorQueueSizePerNodeCount, float64(deps.Protocol.MainEngine.Get().Scheduler.ValidatorQueueBlockCount(block.ProtocolBlock().IssuerID)), block.ProtocolBlock().IssuerID.String())
				}
			}, event.WithWorkerPool(Component.WorkerPool))
	...
	collector.WithMetric(collector.NewMetric(manaAmountPerNode,
		...
		collector.WithInitFunc(func() {
			deps.Protocol.Events.Engine.Scheduler.BlockEnqueued.Hook(func(block *blocks.Block) {
				mana, err := deps.Protocol.MainEngine.Get().Ledger.ManaManager().GetManaOnAccount(block.ProtocolBlock().IssuerID, block.SlotCommitmentID().Slot())
				if err != nil {
					deps.Protocol.MainEngine.Get().ErrorHandler("metrics")(ierrors.Wrapf(err, "failed to retrieve mana on account %s for slot %d", block.ProtocolBlock().IssuerID, block.SlotCommitmentID().Slot()))
	...
	collector.WithMetric(collector.NewMetric(basicBufferReadyBlockCount,
		...
		collector.WithCollectFunc(func() (float64, []string) {
			return float64(deps.Protocol.MainEngine.Get().Scheduler.ReadyBlocksCount()), []string{}
		}),
	)),
	collector.WithMetric(collector.NewMetric(basicBufferTotalSize,
		...
		collector.WithCollectFunc(func() (float64, []string) {
			return float64(deps.Protocol.MainEngine.Get().Scheduler.BasicBufferSize()), []string{}
		}),
	)),
	...
	collector.WithMetric(collector.NewMetric(validatorBufferTotalSize,
		...
		collector.WithCollectFunc(func() (float64, []string) {
			return float64(deps.Protocol.MainEngine.Get().Scheduler.ValidatorBufferSize()), []string{}
		}),
	)),
